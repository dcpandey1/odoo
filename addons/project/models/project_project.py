...
class Project(models.Model):
    ...
    closed_task_count = fields.Integer(
        compute="_compute_closed_task_count", export_string_translation=False
    )
    task_completion_percentage = fields.Float(
        compute="_compute_task_completion_percentage", export_string_translation=False
    )
    ...
    rating_status = fields.Selection(
        [("stage", "when reaching a given stage"), ("periodic", "on a periodic basis")],
    ...
    update_ids = fields.One2many(
        "project.update", "project_id", export_string_translation=False
    )
    update_count = fields.Integer(
        compute="_compute_total_update_ids", export_string_translation=False
    )
    ...
    last_update_color = fields.Integer(
        compute="_compute_last_update_color", export_string_translation=False
    )
    milestone_ids = fields.One2many(
        "project.milestone", "project_id", copy=True, export_string_translation=False
    )
    ...
    def _compute_next_milestone_id(self):
        ...
        for project in self:
            milestone = milestone_ids_per_project_id.get(
    ...
    def copy_data(self, default=None):
        ...
        if default and "name" in default:
            return vals_list
        return [
            dict(vals, name=self.env._("%s (copy)", project.name))
    ...
    def copy(self, default=None):
        ...
        new_projects = super(Project, self.with_context(copy_context)).copy(
            default=default
        )
        if "milestone_mapping" not in self.env.context:
            self = self.with_context(milestone_mapping={})
        actions_per_project = dict(
            self.env["ir.embedded.actions"]._read_group(
                domain=[
        ...
        for old_project, new_project in zip(self, new_projects):
            ...
            if shared_embedded_actions:
                ...
                for original_action, copied_action in zip(
                    shared_embedded_actions, copy_shared_embedded_actions
                ):
    ...
    def create(self, vals_list):
        ...
        if len(self.env.companies) > 1 and self.env.user.has_group(
            ...
            stage = (
                self.env["project.project.stage"].browse(
                    self._context["default_stage_id"]
                )
                if "default_stage_id" in self._context
                else self._default_stage_id()
            )
    ...
    def write(self, vals):
        ...
        if "allow_task_dependencies" in vals and not vals.get(
            ...
            self.env["project.task"].search(
                [("project_id", "in", self.ids), ("state", "=", "04_waiting_normal")]
    ...
    def message_unsubscribe(self, partner_ids=None):
        ...
        if partner_ids:
            self.env["project.collaborator"].search(
                [("partner_id", "in", partner_ids), ("project_id", "in", self.ids)]
            ).unlink()
    ...
    def _get_stat_buttons(self):
        ...
        if self.rating_count != 0 and self.env.user.has_group(
            ...
            store.add(
                self,
                {
                    "collaborator_ids": Store.many(
                        self.collaborator_ids.partner_id, only_id=True
                    )
                },
                as_thread=True,
            )
